...
def create_helpfulness_agent(
    ...
    Uses exact logic from the original helpfulness agent.
    """
    # Get tools
    tools = get_default_tools(rag_chain)
    ...
    # Core agent functions (exact logic from original)
    def call_model(state: AgentState) -> Dict[str, Any]:
    ...
    def helpfulness_node(state: AgentState) -> Dict[str, Any]:
        ...
        # Exact prompt template from original
        prompt_template = """
...
{final_response}"""
    ...
    def helpfulness_decision(state: AgentState):
        ...
        # Check loop-limit marker (exact logic)
    ...
    # Build the graph (exact structure from original)
    def build_graph():
        graph = StateGraph(AgentState)
        tool_node = ToolNode(tools)
